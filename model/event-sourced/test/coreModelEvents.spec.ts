...
test("has event that renames a property in a model model", () => {
    const customer = modelFns.newInstance("Customer", modelOptions.withProperty(propertyFns.newInstance(propertyOptions.named("name"))))
    const nameProperty = customer.properties[0]
    const event = coreModelEvents.propertyRenamed(customer.name, nameProperty.id, nameProperty.name, propertyNameFns.newInstance("fullName"))
...
test("has event that adds a relationship to a model", () => {
    ...
    const event = coreModelEvents.relationshipAdded(customRef, addressRef, "one", relationshipNameFns.newInstance("Address"))

    const mutatedCustomer = modelEventDescriptors.applyEvent(customer, event)

    expect(mutatedCustomer.relationships).toEqual([relationshipFns.newInstance("Address", address.id, "one")])
})