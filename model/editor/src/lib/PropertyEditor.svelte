...
<form>
    ...
    <select on:change={propertyTypeChanged} class="property-type">
        ...
        {#each propertyDescriptors.list() as propertyDescriptor}
            <option value={propertyDescriptor.propertyType.type}
                    selected={propertyTypeFns.equals( property.propertyType, propertyDescriptor.propertyType, )}>{propertyDescriptor.name.name}</option>
        {/each}
    </select><br/>

    <label class="label">
        <input type="checkbox" name="checkbox" value="text" checked={property.required} class="required-toggle"
    ...
    <label class="label">
        <input type="checkbox" name="checkbox" value="text" bind:checked={property.unique} class="unique-toggle"/>
        Unique
    </label>
