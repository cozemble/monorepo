<script lang="ts">
    ...
    function getTerminatingKey(submitEvent: KeyboardEvent | null): {terminatingKey:'Enter' | 'Tab' | null, shiftKeyPressed: boolean} {
    ...
    function changeHandler(newValue: any | null, submitEvent: KeyboardEvent | null) {
        const {terminatingKey, shiftKeyPressed} = getTerminatingKey(submitEvent)
        if (newValue !== value) {
