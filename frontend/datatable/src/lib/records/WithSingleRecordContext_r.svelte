...
const errorVisibilityByRecordId =
  modelRecordsContextFns.getErrorVisibilityByRecordId()
...
if (!maybeExistingEditor) {
  const eventSourcedRecords =
    modelRecordsContextFns.getEventSourcedRecordGraph()
...
let maybeExistingErrorsForRecord =
  singleRecordEditContext.optionalErrorsForRecord()
...
//
const maybeExistingErrorVisibility =
  singleRecordEditContext.optionalErrorVisibilityForRecord()

if (!maybeExistingErrorVisibility) {
  const errorVisibility = derived(
