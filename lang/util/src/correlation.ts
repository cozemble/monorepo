...
function newCorrelatedRequest(session: CorrelatedSession, prefix = "", correlationRequestId = uuid()): CorrelatedRequest {
    return {_type: "correlated.request", session, correlationRequestId: maybePrefix(prefix, correlationRequestId)}
}

function newCorrelatedSession(prefix = "", correlationSessionId = uuid()): CorrelatedSession {
    return {_type: "correlated.session", correlationSessionId: maybePrefix(prefix, correlationSessionId)}
}
