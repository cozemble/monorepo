...
export function findMandatory<T>(array: T[], predicate: (t: T) => boolean, message = "Did not find item in array"): T {
...
export function compare<T = any>(left: T[], right: T[], comparableProducer: (t: T) => any = (t: T) => t): { leftOnly: T[], both: T[], rightOnly: T[] } {
    let leftOnly = left.filter(aLeft => right.find(aRight => comparableProducer(aLeft) === comparableProducer(aRight)) === undefined)
    let rightOnly = right.filter(aRight => left.find(aLeft => comparableProducer(aLeft) === comparableProducer(aRight)) === undefined)
    let both = uniqueBy([...left, ...right], comparableProducer).filter(aBoth => leftOnly.indexOf(aBoth) === -1 && rightOnly.indexOf(aBoth) === -1)
