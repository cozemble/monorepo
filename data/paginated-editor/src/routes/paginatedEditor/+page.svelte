<script lang="ts">
    ...
    const localHasuraClient = new AxiosGraphQlClient('http://localhost:8080/v1/graphql')
    ...
    const noOpEditorHost: PaginatedEditorHost = {
        async recordEdited(editedRecord: EventSourcedDataRecord): Promise<RecordSaveOutcome> {
            records = records.map(r => r.id.value === editedRecord.record.id.value ? editedRecord.record : r)
...
<div class="persistence-method-container" on:change={persistenceMethodChanged}>
    ...
    <select id="persistenceSelect">
        ...
        <option value="localHasura" selected={selectedHost === localHasuraEditorHost}>Local Hasura
