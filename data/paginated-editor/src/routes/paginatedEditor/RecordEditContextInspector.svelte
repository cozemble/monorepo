...
<div style="margin-left: {indent}px;">
    {#each editContext.eventSourcedRecord.events as editEvent, index}
        <div>
            <a href="#!" on:click={() => showEvent(index)}>{index}. {editEvent._type}</a> {eventSummary(editEvent)}
        </div>
    {/each}

    {#if showEventIndex >= 0}
        <div>
            <DataRecordEditEventInspector editEvent={editContext.eventSourcedRecord.events[showEventIndex]} />
