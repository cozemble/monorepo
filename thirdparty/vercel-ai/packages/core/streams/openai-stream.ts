import { CreateMessage, JSONValue } from '../shared/types.js'
import { createChunkDecoder, getStreamString } from '../shared/utils.js'
...
import { createStreamDataTransformer } from './stream-data.js'
...
export type OpenAIStreamCallbacks = AIStreamCallbacksAndOptions & {
  ...
  ) => Promise<Response | undefined | void | string | AsyncIterableOpenAIStreamReturnTypes>
}
...
function chunkToText(): (chunk: OpenAIStreamReturnTypes) => string | void {
  ...
  return (json) => {
     ...
     */
    if (isChatCompletionChunk(json) && json.choices[0]?.delta?.function_call?.name) {
      isFunctionStreamingIn = true
      return `{"function_call": {"name": "${json.choices[0]?.delta?.function_call.name}", "arguments": "`
    } else if (isChatCompletionChunk(json) && json.choices[0]?.delta?.function_call?.arguments) {
...
function isChatCompletionChunk(data: OpenAIStreamReturnTypes): data is ChatCompletionChunk {
    ...
    async flush(controller): Promise<void> {
      try {
        const isEndOfFunction =
          !isFirstChunk && callbacks.experimental_onFunctionCall && isFunctionStreamingIn
